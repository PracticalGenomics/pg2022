...
`velociraptor()` function accepts a SingleCellExperiment object
...
```{r}
dec <- modelGeneVarByPoisson( , assay.type="spliced")
hvgs <- getTopHVGs( , n=2500)
```
...
velo.out <- scvelo(, assay.X="spliced", 
                   subset.row= , use.dimred="PCA")
velo.out
```
...
```{r}
library(TSCAN)
mst <- TSCAN::createClusterMST(, outgroup=TRUE)
