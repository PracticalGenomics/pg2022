...
```{r}
line.data <- reportEdges(,, clusters=NULL, use.dimred="TSNE")
line.data
```
...
p <- plotTSNE(sce.nest, colour_by="label") + 
  geom_line(data=, mapping=aes(x=dim1, y=dim2, group=edge))
print(p)
```
...
```{r}
embedded <- embedCurves()
embedded <- slingCurves() # only 1 path.
```
...
```{r}
p <- plotTSNE(sce.sling, colour_by=) +
  geom_path(data=, aes(x=Dim.1, y=Dim.2), size=1.2)
...
```{r}
gg <- plotUMAP(sce.sling4, colour_by=I())
