...
```{r}
tab <- table(Assigned=pred$labels, Cluster=colLabels(sce.pbmc))
...
```{r}
tasic.mat <- counts(sce.tasic)
rownames(tasic.mat) <- rownames(sce.tasic)
tasic.rankings <- AUCell_buildRankings(as.matrix(tasic.mat),
...
```{r}
brain.aucs <- AUCell_calcAUC(brain.gs, tasic.rankings)
```
...
```{r}
AUCell_exploreThresholds(brain.aucs["interneurons",], plotHist=TRUE, assign=TRUE) 
```
...
```{r}
markers.2 <- markers.mam[["2"]]
all.gns <- rownames(markers.2)
is.top <- order(markers.2$median.logFC.cohen, decreasing=TRUE)[1:100]
top.gns <- rownames(markers.2[is.top,])
```
...
entrez.top <- mapIds(org.Mm.eg.db, keys=top.gns, 
    column="ENTREZID", keytype="SYMBOL")
entrez.all <- mapIds(org.Mm.eg.db, keys=all.gns, 
                     column="ENTREZID", keytype="SYMBOL")
```
...
```{r}
library(limma)
go.out <- goana(unique(entrez.top), species="Mm", 
    universe=unique(entrez.all))
```
